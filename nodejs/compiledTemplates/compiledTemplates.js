...
module.exports.chatListOwnerFalse = async (ctx, data, fromUser, nextId, hasHTML, sendable_message) => {
    ...
    ctx.reactions_types.forEach(element => {
        if (element.status == 1) {
            ...
            r_img = '<img src="'+element.ramza_icon+'">';
            var matches = element.ramza_icon.match(/<[^<]+>/);
            if (matches) {
                r_img = element.ramza_icon;
            }
            reactions_html += '<li style="'+first_text+'" class="reaction reaction-'+element.id+'" data-reaction="'+element.name+'" data-reaction-id="'+element.id+'" data-reaction-lang="'+element.name+'" data-post-id="'+nextId+'" onclick="Wo_RegisterMessageReaction(this,\''+element.ramza_small_icon+'\','+element.is_html+');">'+r_img+'</li>';
