...
// Keep modal dismissal consistent across pages loaded normally or through AJAX.
// A click anywhere outside .modal-content closes an ordinary dialog. Call,
// payment, upload-progress and other protected dialogs can keep using Bootstrap's
// backdrop: "static" option or data-ramza-modal-locked="true".
function RamzaModalIsLocked($modal) {
  var instance = $modal.data('bs.modal');
  var options = instance && instance.options ? instance.options : {};
  var backdrop = typeof options.backdrop !== 'undefined' ? options.backdrop : $modal.attr('data-backdrop');
  var keyboard = typeof options.keyboard !== 'undefined' ? options.keyboard : $modal.attr('data-keyboard');

  return backdrop === 'static' ||
    keyboard === false ||
    String(keyboard).toLowerCase() === 'false' ||
    $modal.is('.ramza-modal-locked, [data-ramza-modal-locked="true"]');
}

$(document)
  .off('click.ramzaModalBackdrop')
  .on('click.ramzaModalBackdrop', '.modal.in, .modal.show', function (event) {
    var $modal = $(this);

    if ($(event.target).closest('.modal-content').length || RamzaModalIsLocked($modal)) {
      return;
    }

    $modal.modal('hide');
  })
  .off('keydown.ramzaModalDismiss')
  .on('keydown.ramzaModalDismiss', function (event) {
    if (event.key !== 'Escape' && event.keyCode !== 27) {
      return;
    }

    var $modal = $('.modal.in:visible, .modal.show:visible').last();
    if ($modal.length && !RamzaModalIsLocked($modal)) {
...
function Wo_intervalUpdates(force_update = 0, loop = 0) {
	...
	var ajax_request = {
      ...
      not_in: (typeof Wo_RamzaVisiblePostIds == 'function') ? Wo_RamzaVisiblePostIds() : []
    };
	if (hash_posts == true) {
    ...
    var ramza_has_new_posts = data.count_num > 0 && (hash_posts == true || $('.filter-by-more').attr('data-filter-by') == 'all');
    if (ramza_has_new_posts) {
...
function Wo_GetNewPosts() {
  ...
  $.get(Wo_Ajax_Requests_File(), {
    ...
    if(data.length > 0) {
      ...
      data = Wo_RamzaFilterDuplicateFeedPosts(data);
      if (data.length > 0) {
        $('#posts').prepend(data);
      }
    }
...
function Wo_RamzaVisiblePostIds() {
  var post_ids = [];
  $('#posts, #posts_hashtag').find('.post[data-post-id]').each(function () {
    var post_id = parseInt($(this).attr('data-post-id'), 10);
    if (post_id > 0 && $.inArray(post_id, post_ids) === -1) {
...
function Wo_RamzaFilterDuplicateFeedPosts(html) {
  if (typeof html !== 'string' || html.length === 0 || html === 'Please login or signup to continue.') {
    return html;
  }
  var visible_posts = {};
  $.each(Wo_RamzaVisiblePostIds(), function(index, post_id) {
    visible_posts[post_id] = true;
  });
  var wrapper = $('<div></div>').append($.parseHTML(html, document, true));
  wrapper.find('.post[data-post-id]').each(function() {
    var post_id = parseInt($(this).attr('data-post-id'), 10);
    if (post_id > 0 && visible_posts[post_id]) {
...
function Wo_GetMorePosts() {
  ...
  var not_in_posts = Wo_RamzaVisiblePostIds();
  var page_id = 0;
  var user_id = 0;
  var group_id = 0;
  var event_id = 0;
...
/* Ramza modal bridge: keep Bootstrap 3 modal state compatible with Ramza Light's modern .show modal styling. */
(function ($) {
  if (!$ || !$.fn || !$.fn.modal) {
    return;
  }
  ...
  function showModalElement(element) {
    var $modal = $(element);
    $modal.addClass('show in').attr('aria-hidden', 'false');
    $modal.css({ display: 'flex', visibility: 'visible', opacity: 1 });
  }

  function hideModalElement(element) {
    $(element).removeClass('show in').attr('aria-hidden', 'true').css({ visibility: 'hidden', opacity: 0, display: 'none' });
  }
  ...
  function forceHideModal($modal) {
    ...
    hideModalElement($modal[0]);
    window.setTimeout(clearModalBackdrop, 30);
  }

  function collapseSelector(id) {
    return '[data-target="#' + id + '"],[href="#' + id + '"]';
  }

  function showCollapseElement(element) {
    var $collapse = $(element);
    $collapse.removeClass('collapsing').addClass('collapse show in').attr('aria-expanded', 'true').css({ height: '', display: 'block' });
    if (element.id) {
      $(collapseSelector(element.id)).attr('aria-expanded', 'true').removeClass('collapsed');
    }
  }

  function hideCollapseElement(element) {
    var $collapse = $(element);
    $collapse.removeClass('show in collapsing').addClass('collapse').attr('aria-expanded', 'false').css({ height: '', display: '' });
    if (element.id) {
      $(collapseSelector(element.id)).attr('aria-expanded', 'false').addClass('collapsed');
    }
  }

  function toggleSidebarCollapse(trigger) {
    ...
    if ($collapse.hasClass('show') || $collapse.hasClass('in')) {
      hideCollapseElement($collapse[0]);
    } else {
      showCollapseElement($collapse[0]);
    }
  }

  document.addEventListener('click', function (event) {
    var trigger = event.target.closest && event.target.closest('.tag_sidebar_head[data-toggle="collapse"]');
    if (!trigger) {
      return;
    }
    event.preventDefault();
  ...
  $(document)
    ...
    .on('click', '[data-dismiss="modal"], .modal .close', function () {
    ...
    .on('click', '.modal-backdrop', function () {
      var $modal = $('.modal.show.in, .modal.in, .modal.show').last();
      forceHideModal($modal);
    })
    ...
    .on('click', '[data-toggle="modal"][data-target]', function () {
      ...
      window.setTimeout(function () {
        ...
        if ((modalData && modalData.isShown) || $modal.css('display') !== 'none') {
          showModalElement($modal[0]);
        }
      }, 80);
    })
    .on('show.bs.collapse shown.bs.collapse', '.collapse', function () {
      showCollapseElement(this);
    })
    .on('hide.bs.collapse hidden.bs.collapse', '.collapse', function () {
      hideCollapseElement(this);
    })
    .on('click', '[data-toggle="collapse"][data-target]', function () {
      var target = $(this).attr('data-target');
      window.setTimeout(function () {
        var $collapse = $(target);
        if (!$collapse.length) {
        ...
        if ($collapse.hasClass('in') || $collapse.hasClass('show') || $collapse.css('display') !== 'none') {
          showCollapseElement($collapse[0]);
        }
      }, 120);
    });
...
/* Post Views Viewport Detection and Counting System */
(function ($) {
  if (!$) return;

  var viewedPosts = new Set();
  var pendingPosts = new Set();

  function triggerPostView(postId) {
    if (!postId || viewedPosts.has(postId) || pendingPosts.has(postId)) {
      return;
    }
    ...
    $.ajax({
      ...
      if (data && data.status == 200 && typeof data.views !== 'undefined') {
        $('span[data-post-views="' + postId + '"]').text(data.views);
        $('span[data-post-video-views="' + postId + '"]').text(data.views);
      }
    }).fail(function () {
      pendingPosts.delete(postId);
    });
  ...
  function initPostViewTracking() {
    if (typeof window.IntersectionObserver !== 'undefined') {
      var observer = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            var postId = $(entry.target).attr('data-post-id');
            if (postId) {
      ...
      function observePosts() {
        $('.post[data-post-id]').each(function () {
          var el = this;
          var postId = $(el).attr('data-post-id');
          if (postId && !viewedPosts.has(postId) && !el._viewObserved) {
            el._viewObserved = true;
            observer.observe(el);
          }
        });
      }
      ...
      $(window).on('scroll resize', function () {
        ...
        scrollTimer = setTimeout(observePosts, 250);
      });
      $(document).ajaxComplete(observePosts);
    } else {
      function checkPostsInViewport() {
        $('.post[data-post-id]').each(function () {
      ...
      $(window).on('scroll resize', function () {
        if (scrollTimer) clearTimeout(scrollTimer);
        scrollTimer = setTimeout(checkPostsInViewport, 250);
      });
      $(document).ready(checkPostsInViewport);
      $(document).ajaxComplete(checkPostsInViewport);
    }
  }
