...
function Wo_UserData($user_id, $password = true)
{
    ...
    $cover_parts = explode('.', (string) $fetched_data['cover']);
    $explode2 = end($cover_parts);
    $explode3 = $cover_parts;
    ...
    $avatar_parts = explode('.', (string) $fetched_data['avatar']);
    $explode2 = end($avatar_parts);
    $explode3 = $avatar_parts;
...
function Wo_GetReactedTextIcon($object_id, $user_id, $col = "post")
{
    ...
    if ($sql_query_one) {
        if (mysqli_num_rows($sql_query_one) == 1) {
            ...
            if (!empty($wo['reactions_types'][$sql_fetch_one['reaction']]['ramza_small_icon'])) {
                $reaction_icon = "<div class='inline_post_count_emoji reaction'><img src='{$wo['reactions_types'][$sql_fetch_one['reaction']]['ramza_small_icon']}' alt=\"" . $wo['reactions_types'][$sql_fetch_one['reaction']]['name'] . "\"></div>";
            }
        ...
        foreach ($reactions as $key => $val) {
            ...
            if (!file_exists('./themes/' . $wo['config']['theme'] . '/reaction/like-sm.png')) {
                if ($wo['reactions_types'][$key]['is_html'] == 1) {
                    ...
                    if (!empty($wo['reactions_types'][$key]['ramza_small_icon'])) {
                        $reactions_html .= $first . "<div class='inline_post_count_emoji reaction'><img src='{$wo['reactions_types'][$key]['ramza_small_icon']}' alt=\"" . $wo['reactions_types'][$key]['name'] . "\"></div></span>";
                    }
                }
            } else {
