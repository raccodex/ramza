...
if ($f == 'download_updates') {
    if (Wo_CheckMainSession($hash_id) === true) {
        ...
        if (!empty($_GET['purchase_code'])) {
            ...
            $file          = file_get_contents("http://www.ramza.com/check_for_updates.php?code={$purchase_code}&version=$version&full=true&url=$siteurl", false, stream_context_create($arrContextOptions));
                ...
                if ($check['status'] == 'SUCCESS') {
                    if (!empty($check['versions'])) {
                        foreach ($check['versions'] as $key => $version) {
                            ...
                            //     @fclose($f);
                            // }
                            $updater = file_put_contents('updates/' . $version . '/script.zip', file_get_contents("https://www.ramza.com/get_update.php?code={$purchase_code}&version=$version&full=true", false, stream_context_create($arrContextOptions)));
                            if ($updater) {
